...
final class UpdateChecker {
    ...
    static let releasesAPI = URL(string: "https://api.github.com/repos/VladGavrila/LmR/releases?per_page=30")!
    static let assetName = "LmR.zip"
    ...
    private func performDownloadAndInstall(_ release: Release) async {
        ...
        do {
            ...
            try AppInstaller.install(
                zipURL: zipURL,
                expectedBundleID: Bundle.main.bundleIdentifier ?? "com.vgdev.lmr"
            )
    ...
    private func fetchReleases() async throws -> (latest: Release, all: [ReleaseInfo]) {
        ...
        request.setValue("LmR/\(currentVersionString)", forHTTPHeaderField: "User-Agent")
    ...
    private func makeCombinedRelease(latest: Release, all: [ReleaseInfo], current: SemanticVersion) -> Release {
        ...
        let combined = missing
            .map { entry -> String in
                ...
                let heading = "# LmR \(entry.version.description)"
    ...
    private func downloadZip(_ release: Release) async throws -> URL {
        let destination = FileManager.default.temporaryDirectory
            .appendingPathComponent("LmR-update-\(UUID().uuidString)", isDirectory: true)
