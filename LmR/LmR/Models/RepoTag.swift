...
/// UI-only color tag a user can assign to a repo for grouping/sorting in card
/// view. Mirrors sshCM's `HostTag` — not part of the Foundation-only
/// `LmRModels` SPM target (it depends on SwiftUI's `Color`), so it's covered
/// by building/running the app rather than `swift test`.
enum RepoTag: String, CaseIterable, Identifiable, Codable {
    case red
    case orange
    case yellow
    case green
    ...
    static var defaultOrder: [RepoTag] {
        [.green, .yellow, .orange, .red, .blue, .purple, .gray]
    }
}

extension RepoTag: Transferable {
